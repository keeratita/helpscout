...
/**
 * Retrieve a conversation from conversation id.
 *
 * Sample conversation object can be found in the docs:
 * https://developer.helpscout.com/help-desk-api/conversations/get/
 *
 * @param {Object} conversation
 * @param {Function} callback
 */
...
Conversations.prototype.get = function(id, callback) {
  // conversation = defaults(conversation, { mailbox: { id: this.mailboxId } });
  // debug('requesting creations of a conversation on mailbox %d', this.mailboxId);
  debug('getting a conversation from conversation id %d', id);
  request
    .get('https://api.helpscout.net/v1/conversations/'+ id +'.json')
    .auth(this.apiKey, 'X')
    .end(function(err, res) {
      if (err) return callback(err);
      if (res.statusCode !== 200) return callback(new Error('Bad response: ' +res.text));
      debug('got a conversation from conversation id %d', id);
      return callback(null, res.body);
    });
};